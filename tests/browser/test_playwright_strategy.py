...
logger = AsyncLogger(verbose=True, log_file=None, log_level=LogLevel.DEBUG)
...
async def test_playwright_text_mode():
    ...
    browser_config = BrowserConfig(headless=True, text_mode=True)  # Enable text-only mode
    ...
    try:
        ...
        async with page.expect_request(
            ...
            try:
                ...
                logger.info(f"Image request detected: {request.url}", tag="TEST")
                has_images = True
            except Exception as e:
                logger.error(f"Error loading page: {str(e)}", tag="TEST")
                # Timeout means text mode is working
                has_images = False
...
async def test_playwright_session_management():
    ...
    try:
        ...
        logger.info(
            f"Session 2 still functional after killing session 1, data: {data2}", tag="TEST"
        )
        ...
        return (
            is_same_page and is_same_context and data1 == "test_value1" and data2 == "test_value2"
        )
    except Exception as e:
        logger.error(f"Test failed: {str(e)}", tag="TEST")
        try:
