...
def create_mock_dispatcher(results_map: Dict[str, MagicMock] = None):
    ...
    async def mock_run_urls_stream(crawler, urls, config):
        for url in urls:
            ...
            task_result.result = results_map.get(url, create_mock_result(url)) if results_map else create_mock_result(url)
            task_result.url = url
...
class TestArunManyWithBFS:
    ...
    async def test_bfs_arun_many_failed_pages_included(self):
        ...
        results = await strategy._arun_batch("https://example.com", mock_crawler, config)
    ...
    async def test_bfs_arun_many_no_duplicate_urls(self):
        ...
        async def mock_arun_many(urls, config):
            ...
            for url in urls:
                ...
                result.links = {"internal": [{"href": "https://example.com"}], "external": []}
                results.append(result)
            return results
