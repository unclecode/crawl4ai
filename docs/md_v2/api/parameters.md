...
| **`follow_frames`**        | `bool` (True)                  | Detects single full-viewport `<frameset>` pages (a legacy HTML4 redirect pattern) and automatically navigates to the child frame's URL so the entire pipeline operates on the real content. Similar to how HTTP redirects are followed automatically. Set to `False` if you need the raw frameset HTML. |
