...
| **`follow_frames`**        | `bool` (True)                  | Detects single full-viewport `<frameset>` pages and automatically navigates to the child frame's URL so the entire pipeline operates on the real content. Set to `False` to get the raw frameset HTML. |
